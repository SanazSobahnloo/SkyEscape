...
public class PlaneBehaviour : MonoBehaviour {
	...
	public GameObject brokenPlane, GameOverPanel;
...
{
    // جلوگیری از دوبار اجرا شدن گیم اور
    ...
    if (ScoreManager.Instance != null)
        ScoreManager.Instance.StopAndSaveIfHighScore();

    // ✅ UI: وسط بازی خاموش، گیم اور روشن
    if (inGameUI != null) inGameUI.SetActive(false);
    if (GameOverPanel != null) GameOverPanel.SetActive(true);

    // اگر دیگه لازم نداری، اینو حذف کن.
    // اگر canvasStuff کار دیگه‌ای می‌کنه (مثل انیمیشن/تاخیر)، می‌تونی نگهش داری.
	...
	IEnumerator canvasStuff(){
		...
		for(int i = 0; i <= 10; i++){
			float k = (float) i /10;
			GameOverPanel.transform.localScale = new Vector3(k,k,k);
